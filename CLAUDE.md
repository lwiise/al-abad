...
**Section 2's diagram is driven by the scroll, not by the pointer, and the scroll is a SELECTOR there rather than a scrub.** التحديات used to answer a hover: the reader put a pointer on a row and the two circles took that challenge's state. On a phone there is no hover at all, so the section's whole idea was invisible to most of its traffic and inert on desktop until somebody happened to point at it. Now one ScrollTrigger spans the list, a reading line at **55%** of the window picks whichever row is crossing it, and that row lights up and takes the drawing — walking back up the list when the reader scrolls back. The rows are no longer buttons: nothing there is pressable, so they are `<li>`s with `data-active`, and the marker fill is the only thing that says which one is live.

**What makes it work is the dwell floor, not the line placement.** The list is ~330px tall, so five challenges over its own travel is **~65px of scroll each** — and there is no line placement that widens that without pushing either the first row or the last one off the screen it is supposed to be lighting up (at 50% القلق's top pressure dots arrive already behind the fixed nav on a 768px laptop; higher than 55% and the list's last row is under the fold when its first row lights up). So `MIN_DWELL_MS` — 650ms, the diagram's own `--ch-move` — governs instead: a state may not be replaced before it has finished arriving. **Nothing is buffered**, which is the deliberate half of the trade: when the floor lifts, what lands is the row under the line at *that* moment, so a fast scroll SKIPS challenges rather than falling behind. A reader who scrolls through at speed sees two or three of the five drawings; one who reads sees all five. The alternative — queueing every state so none is missed — puts the drawing on a different challenge from the one the list is highlighting, and that breaks the only contract this section has.
