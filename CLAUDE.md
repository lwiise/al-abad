...
`hero · hero-surface · INK · white · lilac · white · surface · white · white · white · lilac`

**Section 8 (الآراء) is white by owner decision, and the rhythm is weaker for it.** It was the second lilac band; the owner asked for it white, explicitly. The cost is stated here rather than hidden: the page now runs 8 · 9 · 10 as three white bands with no step between them, and the last tone change before that run is section 7's `surface`, which is itself only 1.07:1 against the white on either side of it. So from section 7 to section 11 the page has no band boundary anyone can see, and what separates الآراء from الأسئلة is a `bg-surface` card against a `bg-background` one — the same non-step the next paragraph rules out for *bands*, working here only because a card carries a border and a shadow and a band cannot. If the run ever needs breaking, section 8 is where the lilac came from; do not put it back without asking.

**Do not alternate `background` with `surface`.** #ffffff against #f8f6fb is 1.07:1 — below the threshold of perception, so it produces no rhythm, just a page that looks flat. `lilac` is 1.25:1 against white, which reads. `surface` is for cards and insets, plus the single sanctioned band below (section 7).
...
**Section 7 (الذكاء) is a full-width `surface` band — the one exception to the rule above.** It is the tone the section always had: it used to be an inset panel (`bg-surface` + `border-border` + `shadow-lg` + `.dot-grid`) floating in a white band, and before that a violet `.ai-shimmer` card. The card version shared its formula **verbatim** with section 10's brand CTA — same `rounded-[2rem]`, same `AiOrbit` corner, differing only in hue — so the page said the same thing twice, one screen apart; `.ai-shimmer` was deleted with it, and `AiOrbit` now belongs to `vision` and the course CTA alone. The panel version's problem was different: fencing the section into a column narrower than the page made it read as a widget dropped onto the site. The tone belongs to the section, so the section takes it edge to edge. **This is an owner decision and an exception — do not generalise it into alternating white and `surface` anywhere else.**

Section 6 above is `background`, so that one boundary is 1.07:1 and deliberately faint — and section 8 below is now `background` too, so section 7 is a faint step on *both* sides rather than one. That makes the other three markers carry more than they used to, not less: the `.dot-grid` texture in the gutters outside the content column, the violet **app-icon tile** that opens the section, and the violet **headline tail**. Violet marks the section as **fills on a light ground** — the tile, the "قريباً" chip, the second headline line, the dots on the chips. Moving section 7 to a `lilac` band is the obvious alternative and is still wrong, though not for the reason it used to be: section 8 was lilac and a lilac 7 would have merged the waiting list into the testimonials, and that collision is gone now. What remains is that the owner asked for this tone on this section — see the paragraph above — and that tinting it would make the one section with its own texture and its own hue also the one section with its own ground.

**Sections 7 and 10 read differently now, and section 10 is the only card.** They used to converge (both light cards, "same card, different hue" one screen apart); flattening 7 to a band resolved that, so the remaining distinctions are cheap to keep and worth keeping: **shape** (7 is an open `surface` band, 10 is a `.cta-sheet` wash inside a lilac mat), **hue** (7 is violet, 10 is coral), **the accent line's typeface** (7 sets its tail in violet sans, 10 in Ruqʿah `font-calligraphy` — and Ruqʿah is the hero's device, used a second and *last* time there), and **what fills them** (7 has an app-icon tile, a device mockup and a form; 10 has an eyebrow chip, buttons and faces). If section 7 ever gets boxed again, check the two side by side before shipping. The pill above the headline is shared on purpose — the hero's trust badge is the same shape — so that one is a system, not a duplicate.

### Rules
- **Violet as text is a size rule, not a ban.** #a551fc is 4.05:1 on white and 3.78:1 on surface — large text (24px+, or 18.66px+ bold) and graphics only. Section 7's second headline line is `text-3xl font-bold` on the white band, so it takes `text-highlight`; both grounds are asserted in `check-contrast`, since `surface` is still a card ground. Nothing smaller may be violet on a light ground — the "قريباً" chip and the mockup's source chip set their labels in `text-primary` and keep violet in the fill, the dot and the icon. Same fill-vs-text split `accent-strong` already makes for coral.
- **`.dot-grid` fades inward, never outward.** Its mask is *transparent across the middle and opaque at the rim*, so the dots frame the content instead of sitting behind the type — a full-bleed dot field is the stock SaaS background, and the mask is the only thing that stops it reading as one. Its horizontal stops are `calc(50% ∓ 36rem)`, i.e. exactly half of the `max-w-6xl` content column, so the dots live only in the gutters beside it — and below 72rem, where there are no gutters, the stops clamp and the texture disappears rather than crowding the type. It must be its own layer, never the band: `mask-image` masks `background-color` too, so masking the section would eat its ground along with the dots.
