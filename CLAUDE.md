...
**The mockup takes everything that is left, down to the band's bottom edge.** The band's bottom padding is zero and the drawing is bottom-aligned in a `flex-1` box, so it runs from under the form to the boundary with no plate showing beneath it — there used to be ~100px of empty ground under a shrunken phone. Its scale comes from that budget (`--ai-mock` in `.ai-mock`, globals.css): **0.50 at a 673px window, 0.59 at 768, 0.84 at 865, 1.0 near 975, and 1.37 — 395px wide, larger than it was ever drawn — from 1105 up**, capped there because the copy above it does not grow to match. Each 48px step carries what its band needs at its *shortest* window; slack the ladder cannot use opens above the phone, never below it. Verified **exactly 90vh from 1024×768 up**.

**The cost is the drawing's legibility on short windows, and it is the whole reason this is worth writing down.** The phone's chat text is 12px, so at 0.59 it is ~7px and the source chip reads as a shape rather than as words. Nothing the reader has to *read* is compressed — the subhead, the form, the note and the chips all keep their sizes at every height; the compression is spent on artwork only. Below a ~624px window the ladder stops and the band grows instead. Same on a phone, where the stack is ~1130px against a ~760px budget: 90svh is a floor there, and dropping the `aria-hidden` mockup to force a fit would cost the section its product shot on most of its traffic. If the drawing's text matters more than the exact 90vh on short windows, raise the floor — it is one number per step in `.ai-mock`.
