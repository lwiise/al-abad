...
`hero · hero-surface · INK · white · lilac · white · surface · lilac · white · white · lilac`
...
Section 6 above is `background`, so that one boundary is 1.07:1 and deliberately faint. What marks section 7 alongside the tone: the `.dot-grid` texture in the gutters outside the content column, the violet **app-icon tile** that opens the section, and the violet **headline tail**. Violet marks the section as **fills on a light ground** — the tile, the "قريباً" chip, the second headline line, the dots on the chips. Moving section 7 to a `lilac` band is the obvious alternative and is still wrong: section 8 is already lilac, so it would merge the waiting list into the testimonials.
