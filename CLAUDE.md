...
**It is a one-screen band, and that is why it is two columns from `lg`.** The owner asked for 90vh. As the single centred stack it had been, it measured **1032px** — 840 of content (tile 72, chip 34, headline 94, subhead 59, form 92, mockup 332, gaps 157) under 192 of padding — which is 1.5 screens on a 1366×768 laptop. Tightening the rhythm recovers ~90px and fits only above a ~1000px window; scaling the mockup to close the rest means **58%** at 1366×768, i.e. its 12px chat text at 7px, and the source chip is the one thing that drawing has to say. So the height came out of the layout instead: copy on the start side, mockup and chips on the end side, band height `max(copy, mockup) + padding` rather than the sum. Verified: **exactly 90vh from 1024×768 up**, mockup and type both at full size. Below `lg` it is the stacked centred column it always was, and on a phone the band **grows past 90svh** — a headline, a subhead, an email capture, three chips and a product shot do not fit a 390×844 screen, and dropping the `aria-hidden` mockup to force it would cost the section its product shot on most of its traffic. That trade is stated in `ai-teaser.tsx`; don't undo it by hiding the preview.
