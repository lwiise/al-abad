...
**The pin is measured, not assumed, and on a phone it does not happen.** A sticky stage taller than the window does not scroll inside itself: it holds at the top and its bottom is simply unreachable, so with a ~840px board against a ~660–750px window the last two challenges would light up below the fold. `useStage` measures the board against the window on mount, after the font swap and on every resize; where it does not fit, the section keeps the **unpinned** behaviour — the diagram stuck to the top of the screen, the rows walking past a reading line at **55%** of the window — which is what a phone already had. The line is 55% for two different reasons at once: stacked it has to clear the sticky diagram, and wide it has to leave القلق's dots clear of the fixed nav on a 768px laptop.
