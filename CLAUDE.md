...
`hero · hero-surface · INK · white · lilac · white · white · white · white · white · lilac`

**Section 8 (الآراء) is white by owner decision, and the rhythm is weaker for it.** It was the second lilac band; the owner asked for it white, twice and explicitly. The cost is stated here rather than hidden: sections 6–10 are now five white bands in a row, so between the `lilac` at 5 and the `lilac` at 11 the page has no band-level step at all, and what used to separate الآراء from الأسئلة is a `bg-surface` card against a `bg-background` one. That is the 1.07:1 non-step the next paragraph rules out for *bands* — it works here only because a card carries a border and a shadow and a band cannot. If the run of white ever needs breaking again, section 8 is where the lilac came from; do not put it back without asking.
...
**Section 7 (الذكاء) is a plain white band — no card.** It is full width and `background`, the same sheet sections 4, 6 and 9 are printed on. Two earlier versions boxed it: a violet `.ai-shimmer` card, then a `bg-surface` inset panel with `border-border` + `shadow-lg`. The panel argued that #f8f6fb is invisible as a *band* but legible as a *panel*, since a hairline border and a shadow carry the edge — that was true, and beside the point: fencing the section into a column narrower than the page made it read as a widget dropped onto the site rather than a part of it. The `.ai-shimmer` version had a worse problem, sharing its formula **verbatim** with section 10's brand CTA — same `rounded-[2rem]`, same `AiOrbit` corner, differing only in hue — so the page said the same thing twice, one screen apart. `.ai-shimmer` was deleted with it; `AiOrbit` now belongs to `vision` and the course CTA alone.

What separates section 7 from section 6 — also white — is therefore **not a band tone**. It is three things, and all three are load-bearing: the `.dot-grid` texture in the gutters outside the content column, the violet **app-icon tile** that opens the section, and the violet **headline tail**. Violet marks the section as **fills on a light ground** — the tile, the "قريباً" chip, the second headline line, the dots on the chips. Moving section 7 to a `lilac` band is the obvious alternative and is still wrong — though the reason has changed. It used to be that section 8 was lilac and a lilac 7 would merge the waiting list into the testimonials; section 8 is white now, so that particular collision is gone. What remains is that a band tone was never what section 7 was asking for: the three devices above are what mark it, and tinting it would make the one section with its own texture and its own hue also the one section with its own ground — three signals where one was already enough.

**Sections 7 and 10 read differently now, and section 10 is the only card.** They used to converge (both light cards, "same card, different hue" one screen apart); flattening 7 to a band resolved that, so the remaining distinctions are cheap to keep and worth keeping: **shape** (7 is an open band, 10 is a `.cta-sheet` wash inside a lilac mat), **hue** (7 is violet, 10 is coral), **the accent line's typeface** (7 sets its tail in violet sans, 10 in Ruqʿah `font-calligraphy` — and Ruqʿah is the hero's device, used a second and *last* time there), and **what fills them** (7 has an app-icon tile, a device mockup and a form; 10 has an eyebrow chip, buttons and faces). If section 7 ever gets boxed again, check the two side by side before shipping. The pill above the headline is shared on purpose — the hero's trust badge is the same shape — so that one is a system, not a duplicate.
